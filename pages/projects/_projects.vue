<template>
  <main class="p-8 mx-auto max-w-7xl">
    <section v-if="post">
      <article class="">
        <header
          class="w-full mx-auto md:grid max-w-7xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert md:grid-cols-2"
        >
          <h1 class="mt-4 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">{{ post.title }}</h1>
          <p class="mt-2 text-2xl font-semibold leading-8 text-gray-700">{{ post.subtitle }}</p>
        ...
        <nuxt-content
          :document="post"
          class="w-full mx-auto max-w-7xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert content blog"
        />
      </article>
    </section>
  </main>
